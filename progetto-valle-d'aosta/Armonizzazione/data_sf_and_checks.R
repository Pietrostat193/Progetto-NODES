...
map_vars <- c("vda_edu_terziaria", "totale_presenze")

month_to_plot <- as.Date("2021-12-01")
...
p_1 <- ggplot(map_month) +
  geom_sf(aes(fill = vda_edu_terziaria), color = "white", linewidth = 0.15) +
  scale_fill_viridis_c(na.value = "grey90") +
  labs(
    title = paste("Edu-Terziaria -", format(month_to_plot, "%Y-%m")),
    fill = "Edu-Terziaria"
  ) +
  theme_minimal()

p_2 <- ggplot(map_month) +
  geom_sf(aes(fill = totale_presenze), color = "white", linewidth = 0.15) +
  scale_fill_viridis_c(na.value = "grey90") +
  labs(
    title = paste("Totale Presenze -", format(month_to_plot, "%Y-%m")),
    fill = "Totale Presenze"
  ) +
  theme_minimal()

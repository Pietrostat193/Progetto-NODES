...
# - controlla collinearitÃƒÂ  prima di ogni modello a blocchi
...
#    Creo i lag prima del filtro 2018, cosÃƒÂ¬ gennaio 2018
#    puÃƒÂ² usare il lag 12 del 2017.
# ==========================================================
data_fe <- data %>%
  group_by(comune_key) %>%
...
# ==========================================================
# 8. Helper formule e collinearitÃƒÂ 
# ==========================================================
...
check_collinearity <- function(df, fixed_terms, label, corr_cutoff = 0.70, vif_cutoff = 5) {
  ...
  if (length(fixed_terms) == 0) {
    cat("Nessun predittore fisso: collinearitÃƒÂ  non applicabile.\n")
  ...
  if (length(numeric_terms) >= 1) {
    ...
    if (nrow(inf_vif_tbl) > 0) {
      cat("\nATTENZIONE: VIF infinito tra predittori numerici. Possibile collinearitÃƒÂ  perfetta:\n")
  ...
  # Qui i fattori sono inclusi, perchÃƒÂ© vogliamo solo verificare
  # se il modello complessivo ÃƒÂ¨ stimabile.
  # ----------------------------------------------------------
  ...
  if (rank_deficient) {
    cat("\nATTENZIONE: il design matrix completo ÃƒÂ¨ rank-deficient.\n")
    cat("Questo segnala collinearitÃƒÂ  perfetta o ridondanza esatta nel modello fisso.\n")
  }
...
#     comune_key ÃƒÂ¨ sempre random intercept.
#     Prima di ogni modello con predittori, stampo il check
#     di collinearitÃƒÂ  tra variabili indipendenti.
# ==========================================================
...
#     struttura, meteo e stagionalitÃƒÂ .
# ==========================================================
...
# comparison_table        -> confronto AIC/BIC/R2/singolaritÃƒÂ 

...
script_dir_override <- Sys.getenv("MODELLING_SCRIPT_DIR", unset = "")
...
if (!file.exists(csv_path)) {
  parent_csv_path <- file.path(dirname(script_dir), "data_full.csv")
  if (file.exists(parent_csv_path)) {
...
# - controlla collinearità prima di ogni modello a blocchi
...
#    Creo i lag prima del filtro 2018, così gennaio 2018
#    può usare il lag 12 del 2017.
# ==========================================================
data_fe <- data %>%
  group_by(comune_key) %>%
...
# ==========================================================
# 8. Helper formule e collinearità
# ==========================================================
...
check_collinearity <- function(df, fixed_terms, label, corr_cutoff = 0.70, vif_cutoff = 5) {
  ...
  if (length(fixed_terms) == 0) {
    cat("Nessun predittore fisso: collinearità non applicabile.\n")
  ...
  if (length(numeric_terms) >= 1) {
    ...
    if (nrow(inf_vif_tbl) > 0) {
      cat("\nATTENZIONE: VIF infinito tra predittori numerici. Possibile collinearità perfetta:\n")
  ...
  # Qui i fattori sono inclusi, perché vogliamo solo verificare
  # se il modello complessivo è stimabile.
  # ----------------------------------------------------------
  ...
  if (rank_deficient) {
    cat("\nATTENZIONE: il design matrix completo è rank-deficient.\n")
    cat("Questo segnala collinearità perfetta o ridondanza esatta nel modello fisso.\n")
  }
...
#     comune_key è sempre random intercept.
#     Prima di ogni modello con predittori, stampo il check
#     di collinearità tra variabili indipendenti.
# ==========================================================
...
if (!identical(Sys.getenv("MODELLING_SKIP_BASE_PLOTS", unset = "0"), "1")) {
  par(mfrow = c(2, 2))
...
#     struttura, meteo e stagionalità.
# ==========================================================
...
# comparison_table        -> confronto AIC/BIC/R2/singolarità

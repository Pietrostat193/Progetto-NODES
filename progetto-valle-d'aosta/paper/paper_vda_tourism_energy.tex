...
Model comparisons use likelihood-based fit criteria, mixed-model $R^2$ measures and numerical diagnostics; robustness is also checked using residualized arrivals. Full validation definitions and results are reported in Appendix~\ref{app:method-validation}.
...
where $n$ is the number of observations in the sample used for copula estimation. Thus, each margin is empirical-uniform by construction; diagnostics confirming this property are reported in Appendix~\ref{app:copula-diagnostics}.
...
The copula family, thresholds, shrinkage estimator, bootstrap intervals and external ranking checks are specified in Appendix~\ref{app:tail-validation}. These checks are used to quantify uncertainty and to distinguish a screening index from a calibrated forecast.
...
\section{Appendix: Method validation and copula diagnostics}
\label{app:method-validation}

\subsection{Mean-effect validation}
For each adjacent pair of nested models $M_k \subset M_{k+1}$ we compute the likelihood-ratio statistic
\[
\Lambda_{k,k+1}=2\{\ell(\widehat{\vartheta}_{k+1})-\ell(\widehat{\vartheta}_{k})\},
\]
...
J_i\mid T_i\sim\mathrm{Binomial}(T_i,\pi_i),\qquad

...
output_dir <- file.path(get_script_dir(), "figure")
...
data_flows_path <- c(
  path.expand("~/Desktop/Research/UNVDA/data/data_flows_en.csv"),
  file.path(project_root, "data_flows_en.csv"),
  file.path(project_root, "Modelli", "data_full.csv"),
  file.path(project_root, "Armonizzazione", "data_full.csv")
)

data_flows_file <- data_flows_path[file.exists(data_flows_path)][1]

if (is.na(data_flows_file)) {
  stop("No input dataset found for tourism EDA.")
}

data_flows_en <- read.csv(data_flows_file)

if (!"total_arrivals" %in% names(data_flows_en) && "totale_arrivi" %in% names(data_flows_en)) {
  data_flows_en <- data_flows_en %>%
    rename(
      year_month = date,
      municipality = comune_nome,
      residents = residenti,
...
# ---------------------------
# 0b) Annual arrivals for the whole Valle d'Aosta
# ---------------------------
annual_arrivals_vda <- flows_eda %>%
  group_by(year) %>%
  summarise(total_arrivals = sum(total_arrivals, na.rm = TRUE), .groups = "drop")

plot_annual_arrivals_vda <- ggplot(annual_arrivals_vda, aes(x = year, y = total_arrivals)) +
  geom_col(fill = "#2F6C8F", width = 0.7) +
  geom_text(aes(label = scales::comma(total_arrivals)), vjust = -0.35, size = 3.2) +
  scale_x_continuous(breaks = annual_arrivals_vda$year) +
  scale_y_continuous(labels = scales::comma) +
  labs(
    title = "Annual tourist arrivals in Valle d'Aosta",
    subtitle = "Total arrivals aggregated across all municipalities",
    x = "Year",
    y = "Total arrivals"
  ) +
  theme_minimal() +
...
print(plot_annual_arrivals_vda)

ggsave(
  filename = file.path(output_dir, "annual_tourist_arrivals_valle_aosta.png"),
  plot = plot_annual_arrivals_vda,
  width = 11,
  height = 6,
  dpi = 300
)
